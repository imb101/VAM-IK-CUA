...
namespace UniHumanoid
{
    ...
    {
        public static string ToProperty(string ch)
        {
            switch (ch)
            {
        ...
        public static bool IsLocation( string ch)
        {
            switch (ch)
            {
    ...
    public class BvhNode
    {
        ...
        public virtual void Parse(VAMStringReader r)
        {
        ...
        static string[] ParseChannel(string line)
        {
            var splited = line.Trim().Split();
            ...
            return splited.Skip(2).Select(x => x).ToArray();
        }
    ...
    public class EndSite : BvhNode
    {
        ...
        public override void Parse(VAMStringReader r)
        {
            r.ReadLine(); // offset
        }
    }
    ...
    public class Bvh
    {
            ...
            foreach(var node in Root.Traverse())
            {
                for(int i=0; i<node.Channels.Length; ++i, --index)
                {
                    if (index == 0)
                    {
                        ...
                        {
                            Path = GetPath(node),
                            Property = ChannelExtensions.ToProperty(node.Channels[i]),//.ToProperty(),
                            IsLocation= ChannelExtensions.IsLocation(node.Channels[i]),
                        };
                        return true;
                    }
                }
        ...
        public ChannelCurve GetChannel(BvhNode target, string channel)
        {
            var index = 0;
            foreach (var node in Root.Traverse())
            {
        ...
        static BvhNode ParseNode(VAMStringReader r, int level = 0)
        {
